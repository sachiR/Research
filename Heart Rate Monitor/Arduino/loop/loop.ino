...
void loop(){
  ...
  if(data[0] > (heart_high-.9*(heart_high-heart_low)))//this is true if the read is greater than 95% of the span
  digitalWrite(8, HIGH);
  else
  digitalWrite(8,LOW); 
  
  if(data[0] > (heart_high-.8*(heart_high-heart_low)))// greater than 90% of the span and so on
  digitalWrite(9, HIGH);
  else
  digitalWrite(9,LOW); 
